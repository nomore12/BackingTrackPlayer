...
import { Button, Checkbox, FormControlLabel, Slider, Typography, Box, TextField } from '@mui/material';
...
const MediaPlayer: React.FC = () => {
  ...
  return (
      <Box sx={{ padding: 4, maxWidth: 600, margin: '0 auto', textAlign: 'center' }}>
        ...
        <Box sx={{ marginTop: 3 }}>
          <Button variant="contained" onClick={handlePlay} sx={{ marginRight: 1 }}>
            재생 시작
          </Button>
          <Button variant="contained" onClick={handlePause} sx={{ marginRight: 1 }}>
            멈추기
          </Button>
        ...
        <Box sx={{ marginTop: 3 }}>
          <Typography gutterBottom>재생 위치: {currentTime.toFixed(2)}초</Typography>
          <Slider
              min={0}
              max={duration}
              value={currentTime}
              onChange={(e, value) => handleSeekChange(e as Event, value)}
        ...
        <Box sx={{ marginTop: 3 }}>
          <FormControlLabel
              control={<Checkbox checked={isRepeating} onChange={handleRepeatToggle} />}
              label="반복하기"
          />
        </Box>
        <Box sx={{ marginTop: 3 }}>
          ...
          <Slider
              ...
              max={duration}
              value={pointA ?? 0}
              onChange={(e, value) => handleSetPointA(e as Event, value)}
          />
          <TextField
              type="number"
              value={pointA ?? 0}
              onChange={(e) => handleSetPointA(e as unknown as Event, parseFloat(e.target.value))}
              inputProps={{ step: 0.1, min: 0, max: duration }}
              sx={{ width: 100, marginTop: 2 }}
          />
        </Box>
        <Box sx={{ marginTop: 3 }}>
          ...
          <Slider
              ...
              max={duration}
              value={pointB ?? 0}
              onChange={(e, value) => handleSetPointB(e as Event, value)}
          />
          <TextField
              type="number"
              value={pointB ?? 0}
              onChange={(e) => handleSetPointB(e as unknown as Event, parseFloat(e.target.value))}
              inputProps={{ step: 0.1, min: 0, max: duration }}
              sx={{ width: 100, marginTop: 2 }}
          />
        </Box>
        <Box sx={{ marginTop: 3 }}>
          <FormControlLabel
              control={<Checkbox checked={isABRepeat} onChange={handleABRepeatToggle} />}
              label="A/B 반복하기"
          />
        </Box>
